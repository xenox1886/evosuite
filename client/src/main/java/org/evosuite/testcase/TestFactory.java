...
//					throw new ConstructionFailedException("Cannot currently instantiate type "+type);
    ...
    /**
     * Get a random statement that can be duplicated. Atm only method calls
     *
     * @param test         the test case
     * @param lastPosition the last position at which can be inserted
     * @return a tuple of position and statement found or null if there's no viable statement
     */
    Map.Entry<Integer, Statement> getRandomDuplicatableStatement(TestCase test, int lastPosition) {
        Map<Integer, Statement> candidates = new LinkedHashMap<>();
        for (int i = 0; i <= lastPosition; i++) {
            Statement s = test.getStatement(i);
            if (canBeDuplicated(s)) {
                candidates.put(i, s);
            }
        }
        ...
        return getRandomStatement(candidates);
    }

    private boolean canBeDuplicated(Statement statement) {
        if (statement instanceof MethodStatement) {
            MethodStatement methodStatement = (MethodStatement) statement;
            if (Properties.isMethodUnderTest(((MethodStatement) statement).getMethod().getMethod())) {
                //only methods that aren't the method under test can be duplicated
                return false;
            }
            return true;
        }
    ...
    private Map.Entry<Integer, Statement> getRandomStatement(Map<Integer, Statement> map) {
        int elementIndex = new Random().nextInt(map.size());
        int i = 0;
        for (Map.Entry<Integer, Statement> entry : map.entrySet()) {
            if (i++ == elementIndex) {
                return entry;
            }
        }
